...
export default async function EditEmployeePage({ params, searchParams }: { params: Promise<{ employeeId: string }>; searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  const employee = await prisma.employee.findUnique({
    where: { id: employeeId },
    include: {
      reviews: { include: { period: true, supervisor: { select: { name: true } } }, orderBy: { createdAt: "desc" } }
    }
  });
  ...
  return (
    <OfficeShell user={user} active="/office/employees">
      <div className="page-head">
        ...
        <form action={setEmployeeActiveAction} className="actions">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 620px) minmax(0, 1fr)", gap: 20, alignItems: "start" }}>
        ...
        <section className="table-card">
          <div style={{ padding: "14px 18px", borderBottom: "1px solid #eef0f2", fontWeight: 700 }}>Review history</div>
          <table>
            <thead><tr><th>Period</th><th>Reviewer</th><th>Status</th><th>Overall</th><th></th></tr></thead>
            <tbody>
              {employee.reviews.length === 0 ? <tr><td colSpan={5} style={{ color: "var(--muted)" }}>No reviews yet.</td></tr> : null}
